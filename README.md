...
- **Edit an agent's instructions in-app.** Each agent's `⋮` menu has "Edit instructions", which opens its `CLAUDE.md` — the file Claude Code loads as that agent's standing instructions every session — in a plain editor with a starter template. (This is separate from the short "Role description", which is only the sidebar blurb.) Changes take effect on the agent's next session. An agent with no `CLAUDE.md` yet shows a one-click nudge to add one.
