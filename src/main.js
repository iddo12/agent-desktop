...
// Builds the standalone PowerShell script that performs the "Update &
// restart" flow. It runs DETACHED, in its own console window, AFTER Agent
// Desktop has fully quit - so nothing holds a file handle inside the npm
// package folder while npm does its extract-then-rename-into-place swap.
// That swap racing a live claude.exe is the exact EPERM corruption that
// broke the CLI on 2026-08-23 when the plain in-app button updated with
// background agents still alive; quitting first is what actually removes
// the race rather than just retrying past it.
function buildClaudeUpdaterScript({ npmPath, claudePath, launchVbs, logPath, appPid }) {
  const q = (s) => String(s).replace(/'/g, "''"); // PS single-quote escape
  return [
    "# Auto-generated by Agent Desktop (Update & restart). Safe to delete.",
    "$ErrorActionPreference = 'Continue'",
    "$log = '" + q(logPath) + "'",
    "function Log($m) { $ts = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'; (\"$ts  $m\") | Tee-Object -FilePath $log -Append | Out-Null }",
    "Log '=== Claude Code update & restart ==='",
    "",
    "# 1. Wait for Agent Desktop's main process to exit fully.",
    ...
    "Log ('main PID " + appPid + " still present = ' + [bool](Get-Process -Id " + appPid + " -ErrorAction SilentlyContinue))",
    "",
    "# 2. Drain background claude.exe agents; force-stop any straggler (the app restarts them).",
    "$deadline = (Get-Date).AddSeconds(20)",
    "while ((Get-Date) -lt $deadline) {",
    "  if (-not (Get-Process claude -ErrorAction SilentlyContinue)) { break }",
    "  Start-Sleep -Milliseconds 400",
    "}",
    "foreach ($p in (Get-Process claude -ErrorAction SilentlyContinue)) {",
    "  Log ('force-stopping leftover claude.exe PID ' + $p.Id)",
    "  try { Stop-Process -Id $p.Id -Force -ErrorAction Stop } catch { Log ('  ' + $_.Exception.Message) }",
    "}",
    ...
    "& '" + q(npmPath) + "' install -g '@anthropic-ai/claude-code@latest' 2>&1 | Tee-Object -FilePath $log -Append",
    "Log ('npm exit code: ' + $LASTEXITCODE)",
    "",
    "# 4. Verify with a real version call before relaunching.",
    "$ver = ''",
    "try { $ver = (& '" + q(claudePath) + "' --version 2>&1 | Out-String).Trim() } catch { $ver = 'ERROR ' + $_.Exception.Message }",
    "Log ('claude --version -> ' + $ver)",
    "if ($ver -notmatch '\\d+\\.\\d+\\.\\d+') {",
    "  Log 'FAILED to verify Claude Code after update - NOT relaunching.'",
    "  Write-Host ''",
    "  Write-Host '  Claude Code update could not be verified.' -ForegroundColor Red",
    "  Write-Host ('  Log: ' + $log)",
    "  Write-Host '  Recover: run  npm install -g @anthropic-ai/claude-code@latest  in a terminal,'",
    "  Write-Host '  then reopen Agent Desktop from its usual shortcut.'",
    "  Write-Host ''",
    "  Read-Host 'Press Enter to close'",
    "  exit 1",
    "}",
    "",
    "# 5. Relaunch Agent Desktop exactly like the desktop shortcut does.",
    "Log 'relaunching Agent Desktop'",
    "& 'wscript.exe' '//B' '" + q(launchVbs) + "'",
    "Log 'done.'",
...
// "Update & restart": the safe path for updating the Claude Code CLI. Quits
// the app (and stops every background agent) FIRST, then a detached helper
// does the npm update with nothing locking the package folder, verifies it,
// and relaunches. Requested by Iddo after the plain in-app update kept being
// risky to run while agents were live.
ipcMain.handle("update-claude-code-restart", async () => {
  if (process.platform !== "win32") {
    throw new Error("Update & restart is implemented for Windows only.");
  }
  const confirm = await dialog.showMessageBox(mainWindow, {
    ...
    message: "Update the Claude Code CLI and restart Agent Desktop?",
    detail:
      "This closes every open agent and Agent Desktop itself, updates the CLI " +
      "(npm install -g @anthropic-ai/claude-code@latest) in a separate window, " +
      "then reopens the app. It takes about a minute. A console window will show progress.",
  });
  ...
  const scriptPath = path.join(workDir, "update-claude-code.ps1");
  const logPath = path.join(workDir, "update-claude-code.log");

  // Stop every background `claude --bg` agent first - these outlive the app
  // by design, and any one of them holding a handle in the npm package
  // folder during the swap is the whole failure mode this flow exists to
  // avoid. Best-effort: the detached script also force-kills stragglers.
  try {
    for (const agent of listAgents()) {
      await stopBackgroundAgentForCwd(sessionCwdFor(agent.path));
    }
  } catch (e) {
    /* best-effort - the helper drains/kills leftovers too */
  }
  ...
  fs.writeFileSync(
    scriptPath,
    buildClaudeUpdaterScript({ npmPath, claudePath, launchVbs, logPath, appPid: process.pid }),
    "utf-8"
  );
